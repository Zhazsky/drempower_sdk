...
class DrempowerScNode : public rclcpp::Node {
public:
    DrempowerScNode() : Node("drempower_sc_node") {
        // Declare parameters
        ...
        // Initialize driver
        if (!driver_.open(can_iface)) {
            RCLCPP_ERROR(this->get_logger(), "Failed to open CAN interface %s", can_iface.c_str());
            rclcpp::shutdown();
            return;
        }
        ...
        cmd_sub_ = this->create_subscription<drempower_sdk::msg::MotorCommand>(
            "motor_commands", 10, std::bind(&DrempowerScNode::commandCallback, this, std::placeholders::_1));

        set_zero_sub_ = this->create_subscription<std_msgs::msg::Int32>(
            "set_zero_position", 10, std::bind(&DrempowerScNode::setZeroCallback, this, std::placeholders::_1));

        // Timer for status updates
        auto period = std::chrono::duration<double>(1.0 / update_rate);
        timer_ = this->create_wall_timer(period, std::bind(&DrempowerScNode::timerCallback, this));

        RCLCPP_INFO(this->get_logger(), "Drempower SocketCAN node started on %s (Radians mode)", can_iface.c_str());
    }
    ...
    void commandCallback(const drempower_sdk::msg::MotorCommand::SharedPtr msg) {
        ...
        switch (msg->type) {
            case 0: { // Absolute Angle Position Control
                ...
                if (multi) {
                    for (auto id : ids) {
                        if (!driver_.presetAngle(id, angle_deg, speed_rpm, param, msg->mode)) success = false;
                    }
                    if (success && !driver_.syncTrigger(0x08 + msg->mode, 0)) success = false;
                } else {
            ...
            case 1: { // Speed Control
                ...
                if (multi) {
                    for (auto id : ids) {
                        if (!driver_.presetSpeed(id, speed_rpm, param, msg->mode)) success = false;
                    }
                    if (success && !driver_.syncTrigger(0x0B + msg->mode, 0)) success = false;
                } else {
            ...
            case 2: { // Torque Control
                ...
                if (multi) {
                    for (auto id : ids) {
                        if (!driver_.presetTorque(id, torque, param, msg->mode)) success = false;
                    }
                    if (success && !driver_.syncTrigger(0x0E + msg->mode, 0)) success = false;
                } else {
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<drempower::DrempowerScNode>());
    rclcpp::shutdown();
    return 0;
}